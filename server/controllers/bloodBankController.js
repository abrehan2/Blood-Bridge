...
exports.registerBloodBank = catchAsyncErr(async (req, res, next) => {
  const { name, email, password, licenseNo, contact, longitude, latitude } = req.body
...
exports.loginBloodBank = catchAsyncErr(async (req, res, next) => {
  ...
  if (!licenseNo || !password) {
    return next(new ErrorHandler('Please enter the license number and password', 400))
  }

  const bloodBank = await bloodBankModel.findOne({ licenseNo }).select('+password')

  if (!bloodBank) {
    return next(new ErrorHandler('Your license number or password is incorrect', 401))
  }
  ...
  if (!bloodBank.verified) {
    ...
    return next(new ErrorHandler('Activate your account by clicking the link in the email', 400))
  }
...
exports.updatePassword = catchAsyncErr(async (req, res, next) => {
  ...
  const bloodBank = await bloodBankModel.findById(req.authUser.id).select('+password')
...
exports.updateProfile = catchAsyncErr(async (req, res, next) => {
  ...
  if (req.body.email !== undefined) {
    if (
      (req.body.email === bloodBank.email && bloodBank.emailVerified === true) ||

...
exports.createBloodDonation = catchAsyncErr(async (req, res, next) => {
  ...
  if (!name || !contact || !age || !bloodBank || !bloodGroup || !donationDate || !disease) {
  ...
  const startOfThreeMonthsAgo = moment().utc().subtract(3, 'months').startOf('month').toDate()
  const startOfTwoMonthsAgo = moment().utc().subtract(2, 'months').startOf('month').toDate()
...
exports.updateDonationStatus = catchAsyncErr(async (req, res, next) => {
  ...
  if (
    (bloodDonation.donationStatus === 'Accepted' || bloodDonation.donationStatus === 'Completed') &&
    status === 'Rejected'
  ) {
    return next(
      new ErrorHandler(
  ...
  } else if (status === 'Completed' && bloodDonation.donationStatus === 'Accepted') {

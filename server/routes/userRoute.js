...
router
  ...
  .put(authenticateUser, authorizeRoles('user', 'admin'), updatePassword)
router
  .route('/user/me/update')
  .put(authenticateUser, authorizeRoles('user', 'admin'), updateProfile)
router.route('/user/:id/verify/:token').get(verifyEmail)
router
  .route('/user/email/resend')
  .get(authenticateUser, authorizeRoles('user', 'admin'), resendEmailVerification)
...
router
  ...
  .put(authenticateUser, authorizeRoles('user', 'admin'), deactivateAccount)
router.route('/user/bloodBanks/all').get(authenticateUser, authorizeRoles('user'), getBloodBanks)
router
  .route('/user/review/bloodBank')
...
router.route('/user/reviews').get(authenticateUser, authorizeRoles('user'), getUserReviews);

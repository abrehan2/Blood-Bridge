...
const CompleteProfile = ({ popup, setPopup }: CompleteProfileProps) => {
    ...
    const [giveBlood, setGiveBlood] = React.useState<boolean>(false)
    ...
    const submitData = (data: completeProfileData) => {
        ...
        data.giveBlood = data.giveBlood === 'on' ? 'true' : 'false';
        axios.post(url, data, {
    ...
    const changeGiveBlood = () => {
        setGiveBlood(!giveBlood)
    }
    ...
    return (
        <div className={cx('w-full h-full absolute left-0 top-0 z-10 bg-black bg-opacity-30 hidden items-center justify-center', { '!flex': popup })}>
            <div className='w-3/5 h-[72vh] bg-white rounded-3xl flex flex-col items-center justify-center relative gap-y-8'>
                <X className='absolute top-3.5 right-3.5 cursor-pointer' size={20} color='#000' onClick={handleCancel} />
                ...
                <form onSubmit={handleSubmit(submitData)} className='w-full flex flex-col items-center'>
                    <div className='w-1/2 flex flex-col gap-y-1.5 mb-1'>
                        <div className='flex items-center gap-x-1.5'>
                            <Switch {...register("giveBlood")} checked={giveBlood} onCheckedChange={changeGiveBlood} id='giveBlood' className='data-[state=checked]:!bg-green-500' />
                            <Label htmlFor='activeStatus'>Give & Take Blood</Label>
                        </div>
                        <p className='text-xs font-LatoRegular text-slate-900 text-opacity-50'>This is a one-time setting, if your blood bank gives blood on basis of exchange, kindly turn this switch on.</p>
                    </div>
